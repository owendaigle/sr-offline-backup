...
echo "----------------------"
    ...
    input_string=("$@") # Read all arguments into an array
    ...
    song_link=${song_info_array[0]}
    artist=${song_info_array[1]}
    album=${song_info_array[2]}
    song_title=${song_info_array[3]}
    cover_art_url=${song_info_array[4]}
    DOWNLOAD_DIR=${song_info_array[5]}
    song_link="${song_link//\"/}"
    cover_art_url="${cover_art_url//\"/}"
    cover_art_url="${cover_art_url//[[:space:]]/}"
    ...
    # create file to log the song link, artist, album, title, and cover URL for debugging purposes
    echo "$song_link|$song_title|$artist|$album|$cover_art_url" > "$DOWNLOAD_DIR/song_info.log"
    wget -q -O "$temp_audio_file" "$song_link"
    ...
    if [[ -n "$cover_art_url" ]]; then
        ...
        echo "$cover_art_url" > "$DOWNLOAD_DIR/cover.log"
        wget -q -O "$temp_cover_file" "$cover_art_url"
        if [ $? -ne 0 ]; then
